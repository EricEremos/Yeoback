...
The [focused workspace section](https://www.figma.com/design/olCrS1PxKWJjMiVlGbC3zR?node-id=266-490), completed on 2026-09-24, documents the flat, disclosure-based layout introduced on 2026-09-08: Home, Clean up, Applications, Documents, Growth, Activity, and Reserve, each in Light and Dark, built from the same sidebar component, semantic Appearance variables, and typography. [figma-focused-state.json](../figma-focused-state.json) records every frame node. These frames remain illustrative references; native captures are authoritative for control rendering.
